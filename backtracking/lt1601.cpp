...
You are given an array requests where requests[i] = [fromi, toi] represents an
employee's request to transfer from building fromi to building toi.
...
and 0 respectively. From building 2 we have employee z and they want to move to
building 0. From building 3 we have employee c and they want to move to
building 4. From building 4 we don't have any requests. We can achieve the
requests of users x and b by swapping their places. We can achieve the requests
of users y, a and z by swapping the places in the 3 buildings. Example 2:
...
public:
  ...
  void backtrack(int idx, int cnt, int m, vector<int> &req,
    ...
    backtrack(idx + 1, cnt + 1, m, req, requests);

    req[from]++;
    req[to]--;

    // Skip request
    backtrack(idx + 1, cnt, m, req, requests);
  }

  int maximumRequests(int n, vector<vector<int>> &requests) {
    int m = requests.size();
    vector<int> req(n, 0);
    backtrack(0, 0, m, req, requests);
    return result;
  }
};